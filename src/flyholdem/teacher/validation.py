...
def verify_evaluation(run, policy_path, registered_config=None, require_confirmatory=True):
    ...
    if (result.get('manifest_sha256')!=digest(run/'manifest.json')
            ...
            or policy.get('schema') not in ('teacher-average-policy-v1','teacher-best-response-policy-v1','teacher-potential-boundary-policy-v1','teacher-external-regret-policy-v1','teacher-final-regret-policy-v1','teacher-self-play-regret-policy-v1')
