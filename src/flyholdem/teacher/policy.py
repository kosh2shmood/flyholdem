...
from .nfsp import network, features
...
def policy_implementation_identity():
    ...
    import platform
    import sys
    names = ('flyholdem.teacher.nfsp', 'flyholdem.teacher.policy', 'flyholdem.teacher.memory',
...
class TeacherPolicy:
    ...
    def probabilities(self, observation):
        x = torch.from_numpy(features(observation))
...
def export_policy(agents, output, provenance):
    ...
    record = {'schema': 'teacher-average-policy-v1', 'mode': 'conventional-teacher-control',
              ...
              'feature_version': 'canonical-visible-237-with-public-first-action-v1',
...
def load_policy(output):
    ...
    if (record['schema'] != 'teacher-average-policy-v1' or record['aggregation'] != 'equal-probability-mixture'
            or record['agents'] != 2 or record['feature_version'] != 'canonical-visible-237-with-public-first-action-v1'):
    ...
    models = [network(record['hidden']) for _ in range(record['agents'])]
    ...
    return TeacherPolicy(models), record
