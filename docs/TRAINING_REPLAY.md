...
`--start-hand` is zero-based. `--hands` selects up to 128 consecutive hands; the displayed cumulative return covers that selected segment. The service audits the complete arm before selecting the display range, so startup still reads and checks the entire training log. This bounds the displayed event collection, not the initial audit's memory or work. A future large run should measure that startup cost on its real records.
