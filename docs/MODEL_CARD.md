...
The 25k/250k v1, visible-card v2, visible-equity v3 and Double DQN v4 full 20 BB NFSP candidates failed their complete held-out suite. V5 population training and the separately extracted V6 final Q-component mixture also failed the same suite. V7 public-stack potential training also failed the full suite. V8’s final Q-component mixture also failed. V9’s exact terminal fold-value correction produced identical paired-deal records and also failed. V10 external-sampling tabular regret also failed: only random and calling station had positive suite-adjusted lower bounds. A duration-only V11 run completed all 100,000 traversals; its first 30,000 journal records exactly reproduce V10. Its fixed average policy also failed the original development suite, with positive adjusted lower bounds only against random and calling station. Independent verification and replay reproduced all 512 paired records. The teacher's 2,226 decisions included 575 checks, 236 calls, 309 half-pot raises and 800 pot raises, alongside 105 folds and 201 all-ins; action variety does not establish strength. A separate V12 final-positive-regret policy is exported (SHA `6b14ca2d9b1dd559fb5de1f8649611a38de488d5eae2f73dc190f97a0367440a`) and also failed the original development suite, again with positive adjusted lower bounds only against random and calling station. Its 512 actual paired records reproduced exactly; it remains unqualified. The independently checked V13 two-player self-play method is registered at 100,000 synchronous iterations, without a trained-policy or strength claim; see SELF_PLAY_TEACHER.md. No full teacher is qualified. A separate tabular CFR reference passed confirmation against all four original opponents in the restricted 10 BB shove/fold subgame; it is qualified only for that subgame, not the full-hand teacher gate. No validated full-hand teacher/corpus or poker-trained student exists yet. Whole Gate 2A and poker Gates 3–6 remain pending. The full model shown in the dashboard and available for human play is the cue-conditioned model, **unvalidated for poker**.
