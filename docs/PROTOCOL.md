...
## M2 numerical runtime

The separate `SparseBrain` runtime implements the DOOMFLY-comparable analytic LIF model at exactly 0.1 ms. Rest/reset -52 mV-equivalent, strict threshold > -45, membrane tau 20 ms, conductance tau 5 ms, transmission delay 1.8 ms, refractory 2.2 ms. Each tick updates refractory/integration/threshold, then synaptic delivery, then resets new spikes. Both voltage and conductance freeze while refractory, and incoming writes are discarded. Native C++ retains all CSR edges and lazily advances neurons only when exact subthreshold bounds permit. No activity-threshold edge pruning is used.

The independently written scalar Python reference and the native solver passed spike-exact comparison with absolute 0.002 mV-equivalent state tolerance, at 0.1 ms and 10 ms boundaries. Both also passed an independent Brian2 oracle with excitatory/inhibitory convergence, an autapse arriving while refractory, silence and changing drives. The application remains Python 3.11. Brian2's current 2.10.1 release requires Python 3.12; it is therefore isolated in oracle/ with its own lock. This avoids the older Python 3.11 Brian2 release's incompatibility with NumPy 2.4. `make test-oracle` reproduces the two oracle tests.

`make build-kernel` records source SHA-256, C++17 compiler, optimization/float flags and binary SHA-256. Loading refuses mismatched source or binary. Atomic checkpoint generations save every provided mutable array plus JSON state, fsync before publishing pointers, verify hashes/shapes/dtypes and run identity, and retain the latest three generations plus the best registered validation checkpoint. The native complete-state test includes delayed spikes, lazy-evolution cursors, active frontier, weights, membrane/conductance, drive and refractory state. Training runners must additionally save their encoder/decoder/plasticity/RNG/game state. No full-data runtime or conditioning result exists yet.

## M3 controllability registration

`configs/controllability.yaml` fixes annotation classes, seed, a balanced 32-cell fanout for each symbolic channel, five disjoint three-cell MBON ensembles, timing, stimulus candidates and the success threshold before poker. Ensembles use farthest cosine anchors and their nearest unused readouts in the direct KC-contact profile, with at least eight presynaptic inputs. Fixed input interventions rank KC contacts by contrast to the other four group profiles. They are direct interventions on the declared input population, not poker actions or an observation-to-action bypass.

Development tests the ordered gain × input-count candidates and stops at the first candidate reaching 95% for every action. That candidate is frozen to a hashed preregistration before separate confirmatory seeds (20 trials/action), shuffled-input controls and black input. Scores are neural rates divided by a fixed 100 Hz; the resting no-input baseline is zero. Failure is published and blocks dependent claims. Both phase and mode remain explicit. Neural trial logs flush/fsync per trial; complete state checkpoints save initially, every five minutes, and on exit. Resume verifies source/config/graph/binary/environment and reuses only complete independently seeded trials.

### Full-graph gain sensitivity v2

V1 full calibration failed while the circuit passed. Before further trials, register the ordered full-model scale candidates 0.5, 0.25 and 0.1 relative to the original 0.275 contact gain. The same positive factor multiplies every original signed edge; no edge or neuron is removed, and signs, annotation-based readout membership and symbolic projection stay fixed. Runtime and base graph hashes are both recorded. These are alternative declared dynamics assumptions, not learned synapses.

Each scale has a committed configuration and separate development/confirmation seeds. Use the same ordered direct-drive gains (12, 20, 32), with control-cell counts (32, 64, 128), and stop the scale sequence at the first configuration passing its own fixed 95% per-action confirmation. Publish all attempted failures. No poker state, profit or opponent is used. This targeted engineering recovery is not evidence of learning.

Failed calibrations now also record black input and shuffled-input baselines at their first registered stimulus candidate. V1's missing full baselines are supplied in a separate supplementary run that verifies the historical failed config, graph and selected cells, and records links to its unchanged original manifest/result hashes. It cannot turn the failed calibration into a pass.
